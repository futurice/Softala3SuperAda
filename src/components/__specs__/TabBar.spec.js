...
const tabs = {
  index: 0,
  routes: [{ title: 'Tab 1' }, { title: 'Tab 2' }, { title: 'Tab 3' }],
};
...
describe('<TabBar />', () => {
  it('should render a <TabBarButton> for each passed tab', () => {
    ...
    expect(wrapper.find(TabBarButton).map(e => e.props().text)).to.deep.equal([
      'Tab 1',
      'Tab 2',
      'Tab 3',
    ]);
  ...
  it('should mark current tab as selected', () => {
    ...
    expect(
      wrapper.find(TabBarButton).map(e => e.props().isSelected),
